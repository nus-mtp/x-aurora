...
/**
 * Description: A random test case generator that generates random raw text
 * file.
 * 
...
public final class SecurityTestInputGenerator {
    ...
    /**
     * Description: generates the given number of text files (test cases) with
     * given number of lines
     * 
     * @param number,
     *            the number of test cases
     * @param input,
     *            the number of lines per test case
     * 
     * @author GAO RISHENG A0101891L
     */
    private static void generateSingleInput(int number, int input) {
        for (int count = INDEX_ZERO; count < number; count++) {
            ...
            for (int i = INDEX_ZERO; i < input; i++) {
                inputs.add(randomStringLiteral(
                        Math.abs(ran.nextInt() % MAX_STRING_LENGTH)));
            }
    ...
    /**
     * Description: initialize the directory of the generated input and
     * initialize the 'Legal' Literal List
     * 
     * @author GAO RISHENG A0101891L
     */
    ...
    /**
     * Description: Generate the ArrayList that stores all the 'legal'
     * characters
     * 
     * @author GAO RISHENG A0101891L
     */
     ...
     * @return a String which is generated by pseudo-random selection from the
     *         Legal Characters with given size
     * @author GAO RISHENG A0101891L
     */
    private static String randomStringLiteral(int length) {
        ...
        for (int i = INDEX_ZERO; i < length; i++) {
            String temp = legalCharacters
                    .get(Math.abs(ran.nextInt() % legalCharacters.size()));
            while (temp.equals(PATH_SEPARATOR)
                    || temp.equals(NEW_EMPTY_STRING)) {
                temp = legalCharacters
                        .get(Math.abs(ran.nextInt() % legalCharacters.size()));
            }
            output.append(temp);
        }
    ...
    /**
     * @param input,
     *            an ArrayList<String> that stores the generated text file in
     *            lines
     * @param index,
     *            the test case index
     * @author GAO RISHENG A0101891L
    ...
    private static void outputToFile(ArrayList<String> input, int index) {
        try {
            File inputFile = new File(inputDirectory
                    ...
                    + SecurityTestInputGenerator.RANDOM_INPUT_FILENAME + index
